			...
			get {
				...
				if (pictureList != null) {
					foreach (string img in pictureList.Values) {
						...
						allPicturesCount++;
						temp.Add (new UriImageSource { Uri = new Uri ($"http://www.r2builders.fr/boris/data/images/builders/{userId}/{img}"), CachingEnabled = true, CacheValidity = new TimeSpan (5, 0, 0, 0) });
